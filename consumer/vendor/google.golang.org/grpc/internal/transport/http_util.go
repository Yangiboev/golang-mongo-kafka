...
type parsedHeaderData struct {
	...
	// rawStatusCode and rawStatuproductg are set from the raw trailer fields and are not
	// intended for direct access outside of parsing.
	rawStatusCode    *int
	rawStatuproductg string
	httpStatus       *int
...
func (d *decodeState) status() *status.Status {
	if d.data.statusGen == nil {
		...
		d.data.statusGen = status.New(codes.Code(int32(*(d.data.rawStatusCode))), d.data.rawStatuproductg)
	}
...
func (d *decodeState) processHeaderField(f hpack.HeaderField) {
	...
	case "grpc-status":
		...
		d.data.rawStatusCode = &code
	case "grpc-message":
		d.data.rawStatuproductg = decodeGrpcMessage(f.Value)
